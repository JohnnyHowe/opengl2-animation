...
void loadTextures(string path, GLuint &txId)
{
...
void drawVase()
{
	...
	for (int j = 0; j <= 36; j++)
	{
		...
		{
			if (i > 0)
				normal(vx[i - 1], vy[i - 1], vz[i - 1], wx[i - 1], wy[i - 1], wz[i - 1], vx[i], vy[i], vz[i]);
			glVertex3f(vx[i], vy[i], vz[i]);
			if (i > 0)
				normal(wx[i - 1], wy[i - 1], wz[i - 1], wx[i], wy[i], wz[i], vx[i], vy[i], vz[i]);
...
void drawSkybox(GLuint &imgId, float size = 30)
{
	...
	glEnable(GL_TEXTURE_2D);
	glBegin(GL_QUADS);
	glBindTexture(GL_TEXTURE_2D, skyboxId); //Use this texture
...
void initialize(void)
{
	...
	loadTexture("../src/grass.bmp", groundId);
